...
#subroutine for left trimming
sub ltrim { my $s = shift; $s =~ s/^\t+//;       return $s };

#Get all the files in the $folder that start with LIB*

my @FILES = glob("$work/$Syn/$folder/*");
...
#	print OUTPUT "\n$data{$key}{'alt1'}\n$data{$key}{'alt2'}";
	print OUTPUT ltrim("$data{$key}{'alt1'}\t");
}
}
