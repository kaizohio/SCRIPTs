...
source /tgac/software/testing/bin/perl-5.22.1;
source /nbi/software/testing/bin/bioperl-1.6.923; 
...
SCRIPTS_DIR="/nbi/Research-Groups/JIC/Diane-Saunders/kai/SCRIPTS"
...
HOME_DIR="/nbi/Research-Groups/JIC/Diane-Saunders/kai" 
...
#		perl SCRIPTS/Population_STRUCTURE/snpeff_and_structure/compare_snpeff_filtered_to_galaxy20.pl $WORKING_DIR/$SYNONYMOUS_DIR/$line\_snpEff_syn_snps.txt $HOME_DIR/$line/SNPs/20X/$line\_SNP_freq_20x.txt > $WORKING_DIR/$SYNONYMOUS_DIR/Filtered/$line\_snpeff_syn_snps_filt.txt
 #		echo $line " done!"
 #	else
...
#	if [ ! -d $HOME_DIR/References2X ]; then
# 		echo "Reference2x directory not found. Creating one..."
# 		mkdir $HOME_DIR/References2X

# 		if [ ! -f $HOME_DIR/References2X/$line\_Reference_greater_2x.tab ];  then
# 			echo $line "- Reference2x file not found! Creating file..."
#	 		perl $HOME_DIR/SCRIPTS/Population_STRUCTURE/extract_2x_same_ref.pl $HOME_DIR/$line/SNPs/$line\_SNP_ratios.txt > $HOME_DIR/References2X/$line\_Reference_greater_2x.tab
...
#	perl SCRIPTS/Population_STRUCTURE/snpeff_and_structure/combine_snps_filtered_reference3x.pl $HOME_DIR/References2X/$line\_Reference_greater_2x.tab $WORKING_DIR/$SYNONYMOUS_DIR/Filtered/$line\_snpeff_syn_snps_filt.txt >  $WORKING_DIR/$SYNDATA_DIR/$line/$line\_syn_all_data.txt
#	echo $line " - Step 1"
...
# # 	# You need to change the number in $filename[NUMBER] in the following script beforw running it, counting the number of "steps" in the path
	#perl SCRIPTS/Population_STRUCTURE/snpeff_and_structure/generate_Structure_input_new.pl  $WORKING_DIR/$SYNDATA_DIR/$line/Positions.txt  $WORKING_DIR/$SYNDATA_DIR/$line/$line\_Structure_syn_data2.str $line $SYNDATA_DIR $WORKING_DIR
...
cat  $WORKING_DIR/$SYNDATA_DIR/*/*_Structure_syn_data2.str >  $WORKING_DIR/Structure_input/Structure_syn_all_data_same_allele.str
##Add the number to the allele (1,2 for biallelic) to the sample names
##perl $SCRIPTS_DIR/Population_STRUCTURE/snpeff_and_structure/Add_allele_number_Structure_input.pl  $WORKING_DIR/Structure_input/Structure_syn_all_data_same_allele.str  $WORKING_DIR/Structure_input/Structure_syn_all_data.str
 
#Now we will modify the format of the STRUCTURE input file.
 
perl $SCRIPTS_DIR/Population_STRUCTURE/snpeff_and_structure/transpose_matrix_space_delimited.pl  $WORKING_DIR/Structure_input/Structure_syn_all_data_same_allele.str >  $WORKING_DIR/Structure_input/Structure_syn_all_data_flip.str
 
perl $SCRIPTS_DIR/Population_STRUCTURE/snpeff_and_structure/remove_multiallelic.pl  $WORKING_DIR/Structure_input/Structure_syn_all_data_flip.str  $WORKING_DIR/Structure_input/Structure_syn_all_data_biallelic.txt
 
perl $SCRIPTS_DIR/Population_STRUCTURE/snpeff_and_structure/transpose_matrix_space_delimited.pl  $WORKING_DIR/Structure_input/Structure_syn_all_data_biallelic.txt >  $WORKING_DIR/Structure_input/Structure_syn_all_data_biallelic_flip.str
