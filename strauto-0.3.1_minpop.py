...
def kfoldersf():
  mystr = ''.join("mkdir k{}\n".format(x) for x in xrange(minpops, maxpops+1))
  return mystr

## Function to print names of folders to be moved around through shell script.
def mvfolders():
  mvfold = ''.join("k{} ".format(y) for y in xrange(minpops, maxpops+1))
  return mvfold 
...
print "----------------------------------------------------------------------"
print "Minimum number of assumed populations :  ".rjust(55), minpops
...
for myK in xrange(minpops, maxpops+1):
      for run in xrange(1, kruns+1):
	runstr.write("./structure -K %d -m mainparams -e extraparams -o k%d/%s_k%d_run%d 2>&1 | tee log/k%d/%s_k%d_run%d.log" % (myK, myK, dataset, myK, run, myK, dataset, myK, run))
        runstr.write("\n")
